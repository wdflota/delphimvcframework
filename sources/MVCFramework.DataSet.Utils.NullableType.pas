...
unit MVCFramework.DataSet.Utils.NullableType;
...
uses
  System.SysUtils,
  System.StrUtils,  // dFlota
  System.Classes, // dFlota
  Data.DB,
  System.Generics.Collections,
  System.JSON,
  System.Rtti,
  System.TypInfo, // dFlota
  JsonDataObjects,
...
type
  ...
  {$REGION 'dFlota'}
    ...
    public
      ...
      procedure SetNullableValue(AInstance: Pointer; ATypeInfo: System.TypInfo.PTypeInfo;
        AValue: Variant);
    end;
  {$ENDREGION}
...
begin
  ...
  if ATypeInfo = TypeInfo(Nullable<Boolean>) then
    if TVarData(AValue).VType <= varNull then
      ...
      Self.SetValue(AInstance, TValue.From(Nullable<Boolean>.Create(Boolean(AValue))))
end;
...
var
  lSerializer: IMVCSerializer;
  LMVCNameCase: TMVCNameCase; // dFlota
begin
  ...
  try
    ...
    {$REGION 'dFlota'}
      case AFieldNamePolicy of
        fpLowerCase:
          LMVCNameCase := ncLowerCase;
        fpUpperCase:
          LMVCNameCase := ncUpperCase;
        fpAsIs:
          LMVCNameCase := ncAsIs;
      else
        LMVCNameCase := ncCamelCase;
      end;
    {$ENDREGION}
    lSerializer.DeserializeDataSet(AJSONArray, Self, nil, LMVCNameCase);  // Modify ncAsIs for LMVCNameCase
  finally
    Self.EnableControls;
  end;
end;
...
var
  ...
  LBlobField: TBlobField; // dFlota
  LStream: TStream; // dFlota
begin
    ...
    case lRttiProp.PropertyType.TypeKind of
      ...
      tkClass:
        begin
          {$REGION 'dFlota'}
            case LField.DataType of
              ftBlob, ftOraBlob, ftOraClob, ftStream, ftGraphic:
              begin
                LBlobField := (LField as TBlobField);
                LStream := ADataSet.CreateBlobStream(LBlobField, bmRead);
                Value := (LStream as TMemoryStream);
              end;
              ftMemo, ftFmtMemo, ftWideMemo:
              begin
                LStream := TStringStream.Create(LField.AsString, TEncoding.Default);
                Value := LStream;
              end;
            end;
          {$ENDREGION}
      ...
      tkRecord:
        begin
          {$REGION 'dFlota'}
            if lRttiProp.IsNullable then
              lRttiProp.SetNullableValue(AObject as TObject, lRttiProp.PropertyType.Handle, LField.Value);
          {$ENDREGION}
        end;
    else
      Continue;
    end;
    if not lRttiProp.IsNullable then  // dFlota
